    ...
    def get(self, id):
        if INTERNAL_TOKEN.compare(request.headers.get('Authorization')):
            try:
                ...
                if element_data:
                    if (element_data.parent_area_id == -1):
                        del element_data.parent_area_id
                    response = jsonify(element_data.json())
                    response.status_code = HttpStatus.OK
                else:
        ...
        if INTERNAL_TOKEN.compare(request.headers.get('Authorization')):
            ...
            try:
                ...
                if element_data:
                    ...
                    element_data.cvedis = EmptyValues.EMPTY_STRING if request.json['cvedis'] == EmptyValues.EMPTY_STRING else request.json['cvedis']
                    element_data.save()
                    response = jsonify(element_data.json())
                    response.status_code = HttpStatus.CREATED
                else:
